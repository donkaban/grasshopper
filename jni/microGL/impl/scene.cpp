...
{	
	...
	for(auto & obj : render_list)
	{
		...
		glUniformMatrix4fv(mat->uniforms.prj,   1,GL_FALSE,prj_m.data);
		glUniformMatrix4fv(mat->uniforms.iview, 1,GL_FALSE,iview_m.data);
		glUniform1f(mat->uniforms.time, cur_time);
		obj->render(mat);
	}	
}
